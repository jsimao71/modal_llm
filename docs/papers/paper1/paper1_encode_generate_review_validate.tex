...
As of Sunday, August 23, 2026, the first reinjection diagnostics remain negative. Under the same small held-out Z-only strong-goal setting, naive all-layer additive reinjection of a single-vector $Z_G$ performs worse than the input-only condition: exact match falls from .148 to .119, facet satisfaction falls from .608 to .526, the shuffled-goal effect falls from .041 to .027, and the zero-goal effect falls from .059 to .051. A periodic reinjection schedule that reintroduces the same latent every two layers partially recovers some of this loss, but does not surpass the input-only baseline: exact match rises only to .121, facet satisfaction to .554, and the shuffled-goal effect to .031, while the zero-goal effect drops further to .020. Held-out linear-probe facet accuracy remains nearly unchanged across these conditions (.643, .639, and .637), so the main failure is not a collapse in decodability alone. The current evidence therefore suggests that repeated additive access by itself is insufficient and can introduce interference unless the access pattern or conditioning mechanism is better matched to generation. The next diagnostic should therefore compare more selective schedules such as late-layer reinjection, or move to a different conditioning mechanism such as prefix-style or KV-based access, before launching wider depth or horizon sweeps. The exact source values are stored in \texttt{results/z\_only\_reinjection\_pilot\_iteration2.json}.
