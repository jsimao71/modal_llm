...
The convergence extension passes the 31-token task and causal gates. Correct Z reaches 1.0 exact match and satisfaction in every output quartile, while shuffled Z falls to .0156 exact match and .503 satisfaction, yielding correct-minus-shuffled effects of .984 and .497. Replacing one facet slot produces the complete counterfactual sequence in .672 of examples and preserves untouched positions at .998 satisfaction; selected-facet positions reach .703. Thus the compact latent channel supports perfect scheduled generation and strong localized control once it has explicit slot identity, generation position, and sufficient optimization. Localized control is not yet perfect, and epoch 120 remains selected despite validation language-model loss reaching .0068, so this remains a one-seed mechanism diagnostic. It justifies the next approximately 64-token point with matched direct and shuffled controls, not the full grid or headline replication. That point uses ten independently shuffled six-facet blocks, yielding 61 output tokens including the end marker. The direct-text gate does not converge within 120 epochs: exact match is zero, facet satisfaction is .678, and validation loss is still falling rapidly at the selected final epoch (.473). A deterministic rerun with only the ceiling raised to 160 exactly reproduces the epoch-120 loss and then passes the gate: exact match and every quartile reach 1.0 with zero drift, while validation loss falls to .00194. Thus the initial failure reflects delayed optimization rather than demonstrated horizon capacity.

Matched signaled facet-slot Z does not pass the 61-token gate. Correct-Z exact match is zero and satisfaction is .160 in nearly flat quartiles (.167, .157, .156, .160), compared with .079 under shuffled Z. The small .081 graded effect establishes residual example-specific use, but not task-level control. One-slot replacement is also non-local: selected and untouched satisfaction are both approximately .163. Unlike direct text at epoch 120, this condition has plateaued by its selected epoch 151 (validation LM 1.121), while facet decoding is excellent (validation goal loss .00194). The failure therefore lies between decodable facet slots and their repeated generative use, not in encoder supervision or progressive late-only forgetting. We stop the horizon grid and next isolate repeated-use and generation-time injection at this boundary. Exact values are stored in \path{results/horizon_64_z_facet_slot_signaled_convergence_iteration1.json}; exact direct values are in \path{results/horizon_64_direct_convergence_iteration1.json}.

\begin{figure}[t]
\centering
\includegraphics[width=.78\linewidth]{figures/horizon_persistence_iteration1.pdf}
\caption{One-seed horizon diagnostic. Direct text remains perfect at 61 tokens, while correct-Z generation and slot-swap locality collapse relative to 31 tokens. Shuffled Z is shown to distinguish residual example-specific use from schedule-prior behavior.}
\label{fig:horizon-persistence}
\end{figure}
...
\paragraph{Current gate status and remaining work.}
The one-seed study now establishes strong held-out facet decodability and causal, localized use at 31 tokens, but not persistent or localized use at 61 tokens. Formal frozen-$Z_G$ linear and MLP probes remain necessary because low auxiliary goal loss is only provisional decodability evidence. The immediate next experiment must test facet-aligned persistent prefix-KV or repeated generation-time slot access, together with a control separating repeated slot reuse from absolute generation distance. Zero, constant, and scale-matched random-$Z_G$ interventions should then complete the final-interface necessity audit. The direct-exposure curve, horizons beyond 61 tokens, depth and injection sweeps, layerwise analysis, joint depth--time grid, encode--generate gradient diagnostic, validator comparison, and multi-seed study remain deliberately gated. Thus the present result identifies a failure boundary for one-shot signaled slot prefixes; it does not establish a general failure of persistent goal state.
